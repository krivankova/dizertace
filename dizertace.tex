...
CVA \cite{zhu2007} \cite{pykhtin2010} \cite{gregory2010} \cite{brigo2014}   zahrnutí Wrong-Way do Monte Carlo simulace \cite{hull2012cva} přehlwdový článek \cite{arora2012}
...
Nejrozšířenější modifikací Wienerova procesu splňující tyto požadavky je Geometrický Wienerův proces, který zavedl Paul Samuelson \cite{samuelson1964rational}.
...
\begin{definice}[Geometrický Wienerův proces]
...
\end{definice}
Člen $\mathrm{d}W(t)$ je infinitezimální přírůstek Wienerova procesu a $\mathrm{d}t$ je infinitezimální změna času.
...
\section{It\^oův integrál a It\^oovo lemma}\label{Ito_integral}
...
\end{definice}
Trajektorie jednoduché funkce je po částech konstantní. 

\begin{definice}
Nechť $S$ je jednoduchá funkce 
a zaveďme označení \linebreak$\Delta W_k=W(t_{k+1},\omega)-W(t_k,\omega)$.
Pak
...
Tento důležitý vztah stochastické analýzy můžeme považovat za obdobu Taylorova rozvoje v integrálním počtu.
...
%\chapter{Stochastické modelování úrokových sazeb}
