...
Poznamenejme, že v případě, kdy koeficient $\beta(t_n,X_n)=\beta(t_n)$ nezávisí na procesu $X_n$, pak $\beta'(t_n,X_n)=0$ a Milsteinova metoda je identická s metodou Eulerovou.
...
Například simulace využívající distribuční funkce, nebo substituování jednodušší náhodnou veličinou, případně zavedení zjednodušujících předpokladů o funkci volatility $\beta(t_n,\boldsymbol{X}_n)$.
%Nicméně tyto úvahy jsou již nad rámec této práce.

\subsubsection{Řád konvergence}
...
Řekneme, že numerická metoda má silný řád konvergence $\rho$, jestliže platí
\begin{equation}\label{konvergence_vztah}
\mathsf{E}\big(|X(t)-X(t_n)|\big)\leq K_t\Delta^\rho,
\end{equation}
kde $X(t)$ je přesné řešení SDR v čase $t$, $X(t_n)$ značí příslušnou aproximaci řešení v čase $t$ a $\Delta=\max_{0\leq n\leq t_n}\Delta_n$.
\end{definice}
...
Platí tedy, že v obecném případě, kdy se v difuzním koeficientu $\beta$ vyskytuje řešení stochastické diferenciální rovnice, Milsteinova metoda konverguje k přesnému řešení SDR rychleji než Eulerova metoda. %spolu se zmenšováním časového kroku $\Delta_n$.
