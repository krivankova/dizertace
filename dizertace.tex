...
Nechť $\alpha(t,x)$ a $\beta(t,x)$ jsou funkce dvou proměnných $\alpha,\beta:T\times\mathbb{R}\to\mathbb{R}$.
...
X(t_0)&=X_0.\notag
\end{align}
Náhodnou veličinu $X(t_0)=X_0$ nazýváme \textit{počáteční podmínkou SDR} a stochastický proces $X(t)$ splňující vztah \eqref{SDR_reseni}, nazýváme \textit{řešením SDR} na $T$.
\end{definice}
...
\end{equation} 
Pak má soustava stochastických diferenciálních rovnic \ref{SSDR_def} jednoznačné řešení na $T$.
\end{veta}
\begin{proof}
