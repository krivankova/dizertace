...
%Klikatelne odkazy v textu - elektronicka verze
...
%
%
%%Klikatelne odkazy v textu - tistena verze
%  \usepackage{hyperref} % klikatelne odkazy v textu
%  \hypersetup{
...
%Také bych ráda poděkovala XXX za pečlivé pročtení textu.
\\
...
%  \begin{table}[h]
   ...
   $\mathbb{R}$                              &   jednorozměrný prostor reálných čísel  \\
   $\mathbb{R}^n$                              &     $n$-rozměrný vektorový prostor reálných čísel  \\
   $\mathbb{R}^+$                              &     prostor všech kladných reálných čísel \\
...
%   $f(x,t | x_0, t_0)$                               &    probability density of random process $X(t)$ at fixed time $t$ conditioned by its state $x_0$ at time $t_0$ \\
   $X\sim \mathcal{L}(\boldsymbol{\theta})$                                &   náhodná veličina $X$ mající rozdělením pravděpodobnosti $\mathcal{L}$ s vektorem parametrů  $\boldsymbol{\theta}=(\theta_1,\ldots,\theta_n)$ \\ %random variable $X$ having general probability distribution $\mathcal{L}$\\
...
%  \begin{table}[h]
   ...
   $f(t,\cdot)$				& reálná funkce jedné proměnné, tato proměnná je zastoupena tečkou a $t$ je pevně zvolené\\
...
Představíme historii vývoje modelování v teorii portfolia a srovnáme výhody a nevýhody klasického a dynamického přístupu, který využívá stochastické modelování.
...
\end{definice}
Pro označení stochastického procesu budeme využívat také zkrácený zápis $X(t)$ nebo $X_t$.
...
Geometrický Wienerův proces bývá často definován pomocí  stochastické diferenciální rovnice (které budou zavedeny v části \ref{SDR_kap}).
...
Nechť $X(t)$ je It\^oův proces se stochastickým diferenciálem
...
Holandští fyzikové Leonard Salomon Ornstein a George Eugene Uhlenbeck zformulovali jako první mean-reversion proces v~roce 1930 v~publikaci \cite{OrnsteinUhlenbeck1930}. %a~publikovali v~textu \cite{OrnsteinUhlenbeck1930}. 
...
K formulování a řešení SDR je využíván It\^oův kalkul popsaný v části \ref{Ito_kalkul}.
...
\end{equation}
kde první integrál je Lebesgueův a druhý integrál je It\^oův definovaný v části \ref{Ito_integral}.
...
Uvažujme $n$ stochastických procesů $X^1(t),\dots,X^n(t)$, které popisují nějakým způsobem související náhodné jevy, například vývoj cen $n$ různých akcií na kapitálovém trhu. 
...
Dále nechť $\boldsymbol X_0$ je náhodný vektor, který může být závislý pouze na hodnotách Wienerova procesu v čase $t_0$, %nezávislý na trajektorii Wienerova procesu $\boldsymbol W(t)$ 
a platí
...
a $\Delta W_n=\int_{t_n}^{t_{n+1}}\mathrm{d}W(t)=W(t_{n+1})-W(t_n)$ je přírůstek Wienerova procesu $W$ na intervalu $(t_n,t_{n+1})$.
...
Pro ekvidistantní posloupnost platí $t_n=n\Delta$, kde $n=0,\dots,\left\lfloor\frac{T}{\Delta}\right\rfloor$.
...
V části \ref{WP_kap} jsme uvažovali požadavky kladené na chování stochastického procesu, který můžeme využít k modelování cen aktiv, aby byl model smysluplný.
...
Jak už bylo naznačeno v části \ref{WP_kap}, základní model splňující uvedené požadavky je založen na Geometrickém Wienerově procesu. 
...
Stejný předpoklad nalezneme i v Black--Scholesově modelu, který představíme v části \ref{derivaty_kap}.
...
Výnosností a rizikem aktiva se budeme podrobněji zabývat v podkapitole \ref{charakteristiky_portfolia}. 
S modely zahrnujícími bezrizikové aktivum nás seznámí podkapitola \ref{KTP}.
Výnosnost bezrizikového aktiva se nazývá \textit{bezriziková úroková míra}.
Modelování úrokových měr se věnuje podkapitola \ref{model_urok_miry_kap}.
...
\end{equation}
kde $B(t)$ je cena bezrizikového aktiva a $r_f(t)$ je bezriziková úroková míra v~čase $t$.
...
V této podkapitole budeme uvažovat finanční trh s jedním rizikovým aktivem a s jedním bezrizikovým aktivem (obvykle se v tomto případě předpokládá, že se jedná o hotovost).
...
Proto tyto pojmy budeme v dalším textu této podkapitoly chápat jako synonyma a~mohou být volně zaměňovány.
...
Pomocí konceptu představeného v této podkapitole je možno modelovat cenu finančního derivátu.
...
Tato podkapitola čerpá z obsáhlé monografie Interest Rate Models autorů Briga a Mercuria \cite{brigo2007interest} a z učebnice finanční matematiky slovenských autorů Melicherčíka, Olšarové a Úradníčka působících na Univerzitě Komenského v~Bratislavě \cite{melichercik}.
...
Jak už jsme zmínili v podkapitole \ref{stoch_model_aktiv}, kvůli zjednodušení výpočtů je běžné aproximovat vývoj cen aktiv spojitým procesem a v takovém případě má smysl i spojité diskontování, i~když reálné chování finančních instrumentů je diskrétní.
\begin{definice}
...
Při oceňování finančních derivátů úrokových měr v podkapitole~\ref{metodyCVA} budeme využívat modely okamžité úrokové míry.
...
Z matematického hlediska je snadnější modelovat křivku forwardových úrokových měr, což využijeme při konstrukci Hull--Whitova modelu v části \ref{H-W_model_kap}.
...
%\end{equation*} 
\end{align*}
Navíc platí $r(t)=f(t,t)$, což využijeme při modelování okamžité úrokové míry pomocí Hull--Whitova modelu v části \ref{H-W_model_kap}.
...
V dalších částech podkapitoly \ref{model_urok_miry_kap} budeme tedy předpokládat stochastické chování úrokové míry.
...
Tento jev se nazývá \textit{mean reversion}, jak už bylo zmíněno v části \ref{OU_kap}. %-- reverze k průměrné hodnotě.
...
Parametry tohoto modelu nejsou závislé na čase, jedná se tedy o Wienerův proces s~driftem (viz Definice \ref{Wieneruv_proces_s_driftem}) 
...
Model, který představíme v této části, uveřejnil původem český matematik Oldřich Vašíček v roce 1977 v článku \cite{vasicek1977equilibrium}, kde ho využil k odvození modelu cen bezkupónových dluhopisů.
...
Daný proces je Ornstein--Uhlenbeckův proces definovaný v části \ref{OU_kap}.
...
Modely uvedené v částech \ref{Vasickuv_model_kap} a \ref{CIR_model_kap} nereflektují aktuální časovou strukturu úrokové míry.
...
Při aplikaci Hull--Whitova modelu v podkapitole \ref{aplikaceCVA_kap} budeme chtít simulovat celou výnosovou křivku $R(t,\cdot)$.
...
Platí, že cena tohoto dluhopisu v~čase $t$ je dána očekáváním v rizikově neutrálním světě %ve světe rizikově neutrálním vůči peněžnímu trhu
...
\begin{definice}
Investujeme-li $Y$ korun do nějakého aktiva $j$ v čase $t$, pak korunová hodnota investice v čase $t+\Delta t$ bude $\big(1+r_j(t,t+\Delta t)\big)Y$, kde  $r_j=r_j(t,t+\Delta t)$ definujeme jako \textit{míru výnosnosti}.  
\end{definice}
 
Relativní výnosnost bývá často označována pouze jako výnosnost.
...
Mezi základní charakteristiky portfolia patří \textit{výnosnost portfolia}, která je dána jako $r_p=\boldsymbol{X}^\mathrm{T}\boldsymbol{r}=\sum_{j=1}^nX_jr_j$ a~je rovněž náhodnou veličinou.   
...
Konstrukcí množiny dostupných (nebo též přípustných) portfolií se bude věnovat část \ref{Markowitz_kap}. 
...
Abychom blíže pochopili obrázky a pojmy používané v podkapitole \ref{KTP}, stručně na tomto místě přiblížíme odvození množiny přípustných portfolií  dle Markowitzovy teorie portfolia.
...
Nyní odvodíme vztah pro váhy tržního portfolia, který dále využijeme při konstrukci modelu v podkapitole \ref{muj_model}.
...
Podrobněji bude tento paradox rozebrán v podkapitole \ref{paradox}.


%%%%%%%%%%%%%%%%%%%%%%%%%%{Ohlson-Rosenbergův paradox}
\section{Ohlson--Rosenbergův paradox}\label{paradox}
V této podkapitole představíme základní poznatky z článku Ohlsona a Rosenberga \cite{ohlson}, který byl publikován v roce 1976 v \textit{Journal of Financial and Quantitative Analysis}.
...
\end{equation}
pro všechny časy $s,t\in \mathcal{T}$  a pro každá aktiva $j$ a $k$.
\end{veta}
...
Optimální portfolio $i$-tého investora bude zahrnovat $\alpha_i(t)a_j+\beta_i(t)b_j$ množství $j$-té akcie.
...
Pak celková hodnota držená všemi investory v $j$-tém aktivu je $A(t)a_j+B(t)b_j$ a musí být rovna tržní hodnotě aktiva.
Platí
...
V této podkapitole jsou shrnuty některé základní poznatky Stochastické teorie portfolia, která byla pro nás inspirací pro studium spojitých modelů v~teorii portfolia uvedených v této práci.
...
Na rozdíl od klasického přístupu, se v~SPT dokonce neklade důraz ani na předpoklad neexistence tržní arbitráže, přičemž se v práci diskutuje o vlastnosti trhu vedoucí k existenci arbitráže. 
...
Tento model budeme konstruovat v podkapitole \ref{muj_model}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Spojitý rovnovážný model s očekávanou výnosností závislou na ceně aktiva}\label{muj_model}
V podkapitolách \ref{paradox} a \ref{john} jsme ukázali, že v teorii portfolia běžně používaný dynamický model není konzistentní.
...
Model, který uvažujeme v této podkapitole, předpokládá rovnováhu na trhu, nicméně očekávaná míra výnosnosti není konstantní v~čase.
...
Nechť $\boldsymbol{P}(t)$ je vektor cen $n$ finančních aktiv.
...
\end{equation}
Podle předpokladů v teorii portfolia zmíněných v podkapitole \ref{john} je kovarianční matice plné hodnosti, tedy i matice $\boldsymbol{\Sigma}^{-1}$ má hodnost $n$.
...
%Tuto myšlenku využijeme později při tvorbě modelu pro vektor $\boldsymbol{\mu}$.
Proto je přirozené rozdělit modelování dynamiky $\boldsymbol{\mu}$ na dvě části, relativní dynamiku aktiv (danou váhami v tržním portfoliu) a~dynamiku trhu (reprezentující zmíněný skalární násobek).
...
Jak jsme ukázali v podkapitole \ref{vahy_trznihodnota}, váhy tržního portfolia se rovnají relativním tržním hodnotám finančních aktiv v tržním portfoliu, tedy
\begin{equation} \label{market_value}
\boldsymbol{X}(t)=\frac{\boldsymbol{V}(t)}{\boldsymbol{1}^\mathrm{T}\,\boldsymbol{V}(t)},
\end{equation}
kde $\boldsymbol{V}(t)$ je vektor tržních hodnot finančních aktiv na trhu.
Připomeňme, že tržní hodnota aktiva $j$ je dána jako $V_j(t)=P_j(t)N_j(t)$.
Vzhledem ke vztahu mezi tržní hodnotou aktiva a cenou aktiva a vzhledem k rovnicím \eqref{vahy_mu} a~\eqref{market_value} %můžeme konstatovat, že očekávané výnosnosti $\boldsymbol{\mu}(t)$ a ceny aktiv $\boldsymbol{P}(t)$ jsou stejné až na násobek skalárem.
platí
...
Proto budeme předpokládat, že míra výnosnosti trhu $\mu_M(t)$ se řídí Vašíčkovým modelem z~části \ref{Vasickuv_model_kap}
...
%$$
%\hrulefill 
%Dále budeme hledat vztah pro očekávané hodnoty cen finančních aktiv.
...
%\end{equation*}
%Budeme-li opět uvažovat očekávanou hodnotu  cen finančních aktiv dostáváme vztah
%\begin{equation*} 
...
Model zavedený v této podkapitole budeme v další části aplikovat na reálná data.
Naším cílem bude simulovat trajektorie generované tímto modelem.
Jelikož stochastické diferenciální rovnice \eqref{SDE_cen_diag} a \eqref{SDE_cen} není možné řešit analyticky, budeme využívat numerické metody představené v části \ref{numericke_metody_kap}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Aplikace modelu na reálná data}\label{aplikace}

Data vybraná pro analýzu v této části byla získána ze systému Bloomberg.
...
\hline \vspace{-4mm} \\ 
Označení & Název společnosti & Obor podnikání  \\
 \hline \hline  \vspace{-4mm} \\ \vspace{1mm}
    CEZ & ČEZ, a. s.  &největší výrobce elektřiny v~České republice\\ \vspace{1mm}
    O2 & O2 Czech Republic, a.s.  &český telefonní operátor ve vlastnictví společnosti PPF\\ \vspace{1mm}
    KB & Komerční banka, a.s. &bankovní instituce působící na českém kapitálovém trhu z většiny vlastněná finanční skupinou Société Générale\\ \vspace{1mm}
    PM & Philip Morris ČR, a.s. &přední výrobce a prodejce cigaret v~České republice\\ \vspace{1mm}
    Pegas & Pegas Nonwovens, s.r.o. &přední světový výrobce netkaných textilií se sídlem  v~České republice\\ \vspace{1mm}
    CT &  Česká televize &veřejnoprávní televize v~České republice\\ \vspace{1mm}
    NWR &  New World Resources &přední producent černého uhlí ve střední Evropě\\
\hline
\end{tabular}}
\end{table}

Ceny akcií zohledněné v analýze tvoří tříletou časovou řadu z let 2011 až 2014. %31.1.2011-21.2.21014
...
Kvůli přehlednosti jsme obrázek rozdělili na dvě části.
...
Model zkonstruovaný v podkapitole \ref{muj_model} využijeme pro simulace vývoje cen akcií uvažovaných společností.
...
Aplikace modelu uvedená v této části má sloužit pouze jako ukázka práce s modelem, proto si dovolíme udělat několik zjednodušení a zavést některé zjednodušující předpoklady.
...
%I když se jedná o zásadní zjednodušení, ilustrace modelu.

%\begin{figure}[!htbp]
%  \centering 
%	\includegraphics[width=13.5cm, clip, trim= 0 15 25 50]{IMG/return_of_market_v4b.pdf}
%  \caption{Ukázka trajektorie výnosnosti trhu $\mu_M(t)$.}  \label{return_of_market}
%\end{figure}

Pro simulování vývoje ceny akcie použijeme model daný rovnicí \eqref{SDE_cen}.
V~modelu předpokládáme, že kovarianční matice $\boldsymbol\Sigma$ je v čase konstantní a~tudíž i matice volatilit $\boldsymbol\xi$ bude v čase konstantní. %a můžeme ji považovat za parametr modelu.
Odhad parametru $\boldsymbol\xi$ provedeme výpočtem historické volatility z dat. 
Při simulování trajektorií stochastického procesu popisujícího vývoj cen akcií budeme postupně využívat Eulerovu i~Milsteinovu numerickou metodu, které byly popsány v části \ref{numericke_metody_kap}. 
...
%Připomeňme, že Milsteinova metoda vychází z Eulerovy metody a zdokonaluje aproximaci difuzního členu.
...
%Povšimněme si, že pro malý časový krok se obě metody příliš neliší.
...
Oceňování tohoto rizika se budeme věnovat v podkapitole \ref{metodyCVA}.
...
Obě tato rizika v sobě spojuje kreditní riziko protistrany (více viz část \ref{CCR_kap}), jehož modelování je nedílnou součástí výpočtu Credit Valuation Adjustment. 
...
Vztah pro výpočet CVA vychází ze známé formule pro výpočet očekávané úvěrové ztráty (podrobněji se s ní budeme zabývat v podkapitole \ref{srovnani_metod_cva}).
...
Další možností, jak určit PD protistrany, je využití externího ratingu, který přidělují ratingové agentury (což už jsme zmínili v části \ref{CCR_kap}).
...
Metodiku výpočtu CVA uvedenou v podkapitole \ref{metodyCVA} budeme aplikovat v této podkapitole. 
...
Data pro analýzu poskytla banka působící v České republice (dále jen Banka) a obsahují informace o části jejího portfolia finančních derivátů.
Tato část portfolia obsahuje pět úrokových swapů, z nichž každý je uzavřen s~jinou protistranou.
Banka je ve všech pěti případech v pozici prodávajícího (pro bližší vysvětlení této pozice viz část \ref{swap_kap}).
...
nettingID & identifikace protistrany se smlouvou o nettingu \\
...
Tento přístup, kdy kombinujeme sazby PRIBOR a tržní hodnoty IRS, je vynucen skutečností, že PRIBOR sazby jsou dostupné jen pro splatnosti 1 den, 1 a 2 týdny, 1, 2, 3, 6 a 9 měsíců a 1 rok, 
...
Pro modelování úrokové míry budeme používat Hull--Whitův jednofaktorový model představený v části \ref{H-W_model_kap}.
...
Jeden z možných scénářů vývoje tržní hodnoty pěti úrokových swapů z datového souboru vidíme na Obrázku~\ref{SwapPrice}.
\begin{figure}[!htbp]
  \centering 
...
Dalším krokem je výpočet expozice protistrany definované v části~\ref{exposure_kap}.
...
Očekávanou expozici protistrany jsme definovali v části~\ref{exposure_kap} jako střední hodnotu expozice protistrany.
...
Diskontovanou očekávanou expozici protistrany jsme spočítali v~části \ref{simulace_kap} a pravděpodobnost selhání protistrany před časem $t$ jsme určili v části \ref{funkce_PD_kap}.
...
Jedním z cílů práce je nalezení efektivní metody výpočtu kapitálového požadavku k riziku nárůstu CVA, proto v této části budeme srovnávat metodu výpočtu CVA popsanou v podkapitole \ref{aplikaceCVA_kap} s metodou používanou v bance, která poskytla data.
...
V Tabulce \ref{srovnaniCVA} vidíme srovnání výsledků z části \ref{vypocetCVA_kap} s CVA vypočítanými bankou. 
...
%kalibrace Vašíčkova modelu v teorii portfolia
%přidat dynamiku volatility
%přidat kapitalizace do modelu

%
%Přesvědčili jsme se, že
%Potvrdila se domněnka
...
Cílem této práce bylo navrhnout model pro popis chování cen finančních aktiv, %založeného na vztahu mezi váhami portfolia a výnosností finančních aktiv v portfoliu.
...
%Bylo by možné zvolit jiný stochastický proces, který by popisoval chování výnosnosti trhu v kapitole s navrhovaným modelem v teorii portfolia.
...
%
%Při výpočtu Credit Valuation Adjusment pro oceňování úrokových swapů. 
...
%vzájemné srovnání různých modelů úrokové míry. 
...
\bibliographystyle{abbrv_czech_v2}
...
\newblock {\em Mathematical Models and
  Financial Mathematics. Book of short papers}, 35--42, Brno: Masarykova univerzita, 2014.

\item
L.~Křivánková.
\newblock Asset-pricing models in portfolio theory.
\newblock {\em Financial Mathematics in
  Practice II, Book of short papers}, 42--49, Brno: Masarykova univerzita, 2013.

\item
L.~Křivánková.
\newblock Continuous-time models in portfolio theory.
\newblock {\em XX International Conference PDMU--2012 Problems of Decision
  Making under Uncertainties}, 95--104, Brno: Univerzita obrany, 2012.
\item
L.~Křivánková.
...
\newblock {\em Workshop of the Jaroslav Hájek Center and Financial Mathematics in
  Practice I, Book of short papers}, 24--29, Brno: Masarykova univerzita, 2012.

\item
S.~Kafková, L.~Křivánková a M.~Leváková.
\newblock Survival analysis of patients with brain tumour.
\newblock {\em Mathematical Models and
  Financial Mathematics. Book of short papers}, 27--33, Brno: Masarykova univerzita, 2014.
...
V příloze je uvedena ukázka datového souboru analyzovaného v části \ref{aplikace}.
\begin{figure}[!htbp]
  \centering 
...
\newpage
Další část zdrojového kódu používáme ke zobrazení grafů dat a grafů simulací z části \ref{aplikace}.
...
Na závěr uvedeme ukázky z programu pro výpočet Credit Valuation Adjustment  vytvořeného v prostředí \MATLAB a využívaného v podkapitole \ref{aplikaceCVA_kap}.

První část programu načte data o úrokových swapech z datového souboru vytvořeného v aplikaci Microsoft Excel. 
Další část slouží k vykreslení Grafu~\ref{YieldCurve} počáteční výnosové křivky, která se dále využívá při modelování úrokové míry pomocí Hull--Whitova modelu (viz část~\ref{vynosova_krivka_kap}).
...
Potom vykreslí graf vývoje tříměsíční sazby PRIBOR (viz Obrázek \ref{PRIBOR}) a odhadne z ní parametry Hull--Whitova modelu (viz část \ref{simulace_kap}).
\begin{verbatim}
% =============================================================
% Načte data o PRIBOR
% =============================================================
