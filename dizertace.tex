...
K výpočtům a tvorbě grafů použijeme prostředí \MATLAB.
...
Označení & Název proměnné  \\
 \hline \hline
ID & identifikační číslo protistrany\\
...
\subsection{Počáteční výnosová křivka}
...
\subsection{Simulace úrokových měr a oceňování IRS}
...
Nejdříve odhadneme parametry $\alpha$ a $\sigma$ z historických dat tříměsíční sazby PRIBOR za použití \MATLAB funkce \verb+regress+.
Parametr $\theta(t)$ bude odvozen z počáteční výnosové křivky v \MATLAB.
Následně použijeme vztah \eqref{H-W_model} a získáme simulace okamžité úrokové míry.
V \MATLAB k tomu budeme využívat funkci \verb+HullWhite1F+, jejímiž parametry jsou parametry $\alpha$ a $\sigma$ z Hull-Whitova modelu a hodnoty počáteční výnosové křivky z Tabulky \ref{rates}.  
...
Okamžité úrokové míry simulované modelem rozšíříme na celé výnosové křivky s využitím rovnice \eqref{H-W_vynosova_krivka}.
...
Příklad jedné takové simulace vývoje časové struktury úrokové míry ukazuje trojrozměrný plošný graf na Obrázku \ref{YieldCurveEvolution}.
\begin{figure}[!htbp]
  \centering 
...
Pro každý scénář vypočítáme tržní hodnoty IRS v každém datu simulace v budoucnosti.
...
\subsection{Funkce pravděpodobnosti selhání protistrany}
...
%celkovou hodnotu portfolia pro každý scénář naší simulace
\begin{figure}[!htbp]
  \centering 
...
%celkovou expozici portfolia pro každý scénář v naší simulaci
\begin{figure}[!htbp]
  \centering 
