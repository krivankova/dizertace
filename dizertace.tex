...
$X(t,\cdot):\Omega\to\mathbb{R}$ je pro každé $t\in T$ náhodná veličina.
%\begin{itemize}
...
pro všechna $\boldsymbol{t}=(t_1,\dots,t_n)\in T_n$ a všechna $\boldsymbol{x}=(x_1,\dots,x_n)\in \mathbb{R}^n$.
...
Přírůstky stochastického procesu $X(t)$ jsou \textit{nezávislé} právě tehdy, když pro všechny časové posloupnosti $\{t_i\}_{i=1}^N\subseteq T$, kde $t_i<t_{i+1}$ a $N\in\mathbb{N}$, jsou přírůstky $X(t_{i+1})-X(t_i)$ nezávislé náhodné veličiny.
\end{definice}
...
se nazývá \textit{It\^oův integrál} procesu $S$ na intervalu $[0,T]$.
\end{definice}
...
Nechť $\alpha(t,x)$ a $\beta(t,x)$ jsou spojité funkce dvou proměnných $\alpha,\beta:T\times\mathbb{R}\to\mathbb{R}$.
...
X(t_0)&=X_0.\notag
\end{align}
Náhodnou veličinu $X(t_0)=X_0$ nazýváme \textit{počáteční podmínkou SDR} a stochastický proces $X(t)$, daný vztahem \eqref{SDR_reseni}, nazýváme \textit{řešením SDR} na $T$.
\end{definice}
...
je spojitá vektorová funkce $\boldsymbol\alpha:T\times\mathbb{R}^n\to\mathbb{R}^n$.
...
a nechť $\boldsymbol{B}=(\boldsymbol\beta^{1},\dots,\boldsymbol\beta^{m})$ je spojitá maticová funkce $\boldsymbol{B}:T\times\mathbb{R}^n\to\mathbb{R}^{n\times m}$.

Pro $j=1,\dots,m$ rozumíme \textit{soustavou stochastických diferenciálních rovnic (SSDR)} systém
...
Diferenciální tvar SSDR zapíšeme ve formě
...
X^i(t_0)&=X^i_0.\notag
\end{align}
kde $\boldsymbol{W}({t})=(W_1(t),\dots,W_m(t))^\mathrm{T}$ je $m$-rozměrný Wienerův proces.
\end{definice}

\subsubsection{Existence a jednoznačnost řešení}
Teorie o existenci a jednoznačnosti řešení stochastických diferenciálních rovnic a jejich soustav vychází ze stejné teorie pro obyčejné diferenciální rovnice.
Zejména se předpokládají určité vlastnosti koeficientů rovnic jako je lokální Lipschitzova spojitost.
Následující věta shrnuje podmínky existence a jednoznačnosti řešení soustavy stochastický diferenciálních rovnic.
\begin{veta}[O existenci a jednoznačnosti řešení SDR]
Nechť jsou $\boldsymbol\alpha:T\times\mathbb{R}^n\to\mathbb{R}^n$ a $\boldsymbol{B}:T\times\mathbb{R}^n\to\mathbb{R}^{n\times m}$ měřitelné funkce splňující podmínku
...
\end{equation}
pro každé $\boldsymbol x\in\mathbb{R}^n$ a $t\in T$ a pro nějakou kladnou konstantu $C$.
...
\end{equation}
pro každé $\boldsymbol x , \boldsymbol y\in\mathbb{R}^n$ a $t\in T$ a pro nějakou kladnou konstantu $D$.
A nechť $\boldsymbol X_0$ je náhodný vektor, který může být závislý na hodnotách Wienerova procesu $\boldsymbol W(t)$ pouze v čase $t_0$,%nezávislý na trajektorii Wienerova procesu $\boldsymbol W(t)$ 
a platí
\begin{equation}\label{podminka_tri}
\mathrm{E}\left[||\boldsymbol X_0||^2\right]<\infty.
\end{equation} 
Pak má stochastická diferenciální rovnice \ref{SSDR_def} jednoznačné řešení na $T$.
\end{veta}
\begin{proof}
...
Nechť $\{t_n\}_{n=0}^N\subseteq T$ je časová posloupnost, kde pro všechna $n$ platí $t_n<t_{n+1}$.
...
Zobecníme-li Eulerovu metodu pro soustavu $m$ stochastických diferenciálních rovnic \eqref{SSDR_def}, pak vztah pro $i$-tou složku rovnice je definován předpisem
...
\textit{Eulerova aproximace} je spojitý stochastický proces, který v daných časech $\{t_n\}_{n=0}^N\subseteq T$ splňuje vztah \eqref{Euler} a hodnoty mezi těmito časovými body získáme pomocí lineární interpolace.
...
Milsteinova numerická metoda řešení SDE je popsána v knihách Eckharda Platena \cite{platen2010numerical} a Paula Glassermana \cite{glasserman2003monte}.
...
Zobecníme-li Milsteinovu metodu pro soustavu $m$ stochastických diferenciálních rovnic \eqref{SSDR_def}, pak vztah pro $i$-tou složku rovnice je definován předpisem
...
Nechť $f(x)$ je nějaká daná polynomiální funkce a uvažujme diskretizaci času $\{t_n\}_{n=0}^N$.
...
\end{equation}
kde $X(t_n)$ je přesné řešení SDR v čase $t_n$ a $X_n$ značí příslušnou aproximaci řešení v čase $t_n$.
...
\end{equation}
kde  $\Delta=\max_{0\leq n\leq N}\Delta_n$.
Levá strana vztahu \eqref{konvergence_weak_vztah} se nazývá slabá chyba aproximace.
\end{definice}

\begin{definice}\label{konvergence_def}
Uvažujme diskretizaci času $\{t_n\}_{n=0}^N$.
Řekneme, že aproximace \textit{silně konverguje} k řešení v čase $t_n$, jestliže platí
\begin{equation}
\lim_{\Delta_n\to0}\mathsf{E}\big(|X(t_n)-X_n|\big)=0
\end{equation}
kde $X(t_n)$ je přesné řešení SDR v čase $t_n$ a $X_n$ značí příslušnou aproximaci řešení v čase $t_n$.

Řekneme, že numerická metoda má \textit{silný řád konvergence $p$}, jestliže existuje kladná konstanta $K$ taková, že pro všechna $n$ platí
\begin{equation}\label{konvergence_vztah}
\mathsf{E}\big(|X(t_n)-X_n|\big)\leq K\Delta^p,
\end{equation}
kde  $\Delta=\max_{0\leq n\leq N}\Delta_n$.
Levá strana vztahu \eqref{konvergence_vztah} se nazývá silná chyba aproximace.
\end{definice}
Poznamenejme, že silná konvergence implikuje slabou konvergenci pro funkci $f(x)=x$.
...
Platí tedy, že v obecném případě, kdy se v koeficientu volatility $\beta$ vyskytuje řešení SDR, Milsteinova metoda konverguje k přesnému řešení SDR rychleji než Eulerova metoda (spolu se zmenšováním časového kroku $\Delta$).
...
Budeme předpokládat $N_j(t)=N_j$ pro každé aktivum $i$ a všechna $t\in T$. 
Z~věty \ref{T1} plyne
\begin{align*}
