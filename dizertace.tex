...
\phi(t)\,\mathrm{d}P(t)+\psi(t)\,\mathrm{d}B(t)&=1\cdot\mathrm{d}W(t)+1\cdot1=\mathrm{d}W(t).
\end{align*}
...
K derivátu nalezneme jeho replikační portfolio, využijeme vztah \eqref{replikacni_portfolio_vztah} a cenu rizikového a bezrizikového aktiva modelujeme dle modelů představených v Kapitole \ref{stoch_model}. 
...
Než začneme představovat konkrétní modely úrokových měr, definujeme okamžitou úrokovou míru a seznámíme se s pojmem časová struktura úrokové míry, na jejímž vývoji závisí hodnoty finančních derivátů úrokových měr.
...
Úrokové míry se používají k diskontování.

U úrokových měr budeme předpokládat stochastické chování stejně jako u jiných finančních aktiv,
...
Jednofaktorové x dvoufaktorové modely.
Jednofaktorové modely popisují vývoj krátkodobých úrokových sazeb na základě pouze jednoho zdroje tržního rizika.
...
\end{equation}
Mertonův mode tedy předpokládá, že změny úrokové míry jsou z normálního rozdělení.

\subsection{Vašíčkův model}
Vašíček ho v roce 1977 představil v článku \cite{vasicek1977equilibrium} k odvození modelu cen bezkupónových dluhopisů.
Vašíčkův model předpokládá, že okamžitá úroková míra $r$ je charakterizovaná pomocí řešení stochastické diferenciální rovnice, která má tvar
...
Na druhou stranu nevýhodou je to, že Vašíčkův model připouští i záporné úrokové míry. 
