...
\begin{figure}[!htbp]
  \centering 
	\includegraphics[width=12cm, clip, trim= 0 20 25 40]{IMG/WP_v2.pdf}
...
\begin{figure}[!htbp]
  \centering 
	\includegraphics[width=12cm, clip, trim= 0 20 25 50]{IMG/GP_v2.pdf}
...
Zabývat se vztahy mezi akciemi.
především v teorii portfolia.
\begin{definice}
...
\begin{figure}[!htbp]
  \centering 
	\includegraphics[width=12cm, clip, trim= 0 20 25 50]{IMG/WP_2D_v6.pdf}
...
Uvedené požadavky splňuje Geometrický Wienerův proces, 
budeme tedy předpokládat, že proces popisující vývoj ceny aktiva v čase se řídí následujícím modelem
...
Na rozdíl od akcií nebo komodit vykazuje chování úrokových sazeb určité příznačné vlastnosti. %zvláštnosti.
...
Jednofaktorové modely krátkodobých úrokových měr předpokládájí, že okamžitá úroková míra $r$ je charakterizovaná pomocí řešení stochastické diferenciální rovnice, která má tvar
%dvoufaktorove r odvozuji od dvou informaci x, y
\begin{equation}\label{model_urok_miry}
\mathrm{d}r=\mu(t,r)\mathrm{d}t+\sigma(t,r)\mathrm{d}W
\end{equation}
...
\begin{equation}\label{Vasickuv_model}
 \mathrm{d}r = \kappa ( \theta - r_{t})\mathrm{d}t + \sigma \mathrm{d}W
\end{equation}
kde $\sigma$, $\theta$ a $\kappa$ jsou konstanty. 
...
Drift této rovnice je $\mu(t, r) = \kappa ( \theta - r_{t})$, volatilita této rovnice je $\sigma(t, r) = \sigma$. 
...
\mathrm{d}r = \mu(t, r) \mathrm{d}t + \sigma \sqrt{r} (t, r)\mathrm{d}W
\end{equation}
...
Úroková míra v čase $t + \Delta t$ je rovna $r_{t} + \kappa ( \theta - r_{t})\Delta t + \sigma (W_{t + \Delta t} - W_{t})$.
...
Uvažujme o řešitelnosti a počtu řešení soustavy rovnic \eqref{vahy} vzhledem k~$\boldsymbol{\mu}$, kterou upravíme
...
Zaměřme se nyní na vektor stochastických procesů $\boldsymbol{\mu}(t)$ 
Dvě složky
Multiplikativní vztah
\begin{equation} \label{vynos}
\boldsymbol{\mu}(t) =\tilde{\boldsymbol{\mu}}(t)\cdot r(t)
\end{equation}
$\tilde{\boldsymbol{\mu}}$ zachycuje relativní vztah aktiv v portfoliu.
$\sum_{i=1}^n\tilde{\mu}_i=1$
$r(t)$ celý trh dynamicky se mění v čase, zachycuje dynamiku trhu
Dle CAPM platí $\tilde{\boldsymbol{\mu}}=\boldsymbol{X}$

Předpokládáme, že výnosnost je omezená funkce
\begin{equation} 
\mathrm{d}r(t)=\theta(\mu-r(t))\,\mathrm{d}t+\sigma\,\mathrm{d}W(t),
\end{equation}
kde $\theta$, $\mu$ a $\sigma$ jsou konstanty
...
Dosadíme-li vztah \eqref{vynos} do stochastické diferenciální rovnice \eqref{SDE}, pak s ohledem na předpoklady učiněné výše dostáváme
...
V další části představíme model s méně přísnými předpoklady.
Připusťme nyní možnost závislosti mezi jednotlivými aktivy, tedy nechť matice $\boldsymbol{\Sigma}$ není diagonální.
Za těchto předpokladů dosaďme vztah \eqref{vynos} do SDR \eqref{SDE}, čímž obdržíme
\begin{equation} 
 \mathrm{d}P_j(t)=P_j(t)\left(\frac{P_j(t)N_j}{\sum_{i=1}^n P_i(t)N_i}\, r(t)\right)\,\mathrm{d}t+P_j(t)\sum_{k=1}^{n}\xi_{jk}(t)\,\mathrm{d}W_k(t).
\end{equation}

\begin{comment}
\begin{figure}[!htbp]
  \centering 
	\includegraphics[width=12cm, clip, trim= 0 20 25 50]{IMG/weight_of_asset.pdf}
