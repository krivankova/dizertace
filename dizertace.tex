...
Stochastický proces $X(t)$ nazýváme řešením SDR.
...
%Numerická metoda nám určí aproximaci řešení v diskrétních časových hodnotách.
%Potřebujeme-li spojitou aproximaci řešení, použijeme interpolační metody.
\subsubsection{Eulerova metoda}
Uvažujme SDR \ref{SDR_def} a časovou posloupnost $\{t_i\}_{i=0}^N\subseteq T$, kde pro všechna $i$ platí $t_i<t_{i+1}$.
Pak Eulerova metoda řešení SDR je definována rekurzívním vztahem
\begin{equation}
X_{n+1}=X_n+\alpha(t_n,X_n)\Delta_n+\beta(t_n,X_n)\Delta W_n,
\end{equation}
kde $\Delta_n=t_{n+1}-t_n$ a $\Delta W_n=W(t_{n+1})-W(t_n)$.
