...
Podstatou derivátu je termínový obchod, tedy takový, kde doba od sjednání obchodu do jeho vypořádání v budoucnosti má ekonomický význam.
...
Na dalším rozvoji se podílel také Robert Merton a~jejich práce byla v roce 1997 oceněna Nobelovou cenou za ekonomii.
...
Dluhopis je cenný papír, který vyjadřuje závazek emitenta vůči věřiteli, a~je s ním spojeno právo požadovat splacení dlužné částky (tzv. nominální hodnoty) a~vyplacení výnosů v~předem určenou dobu.
...
\end{equation} 
Funkce $R(t,T)$ se nazývá \textit{úroková míra}.
\end{definice}
Je tedy zřejmé, že z cen bezrizikových dluhopisů $B(t,T)$ je možné odvodit ceny úrokových měr $R(t,T)$ a naopak.
Platí
\begin{equation}
...
\begin{equation}\label{okamzik_urok_mira_vztah}
r(t)=\lim_{T\to t}R(t,T)=\lim_{T\to t}-\frac{\log B(t,T)-\log B(t,t)}{T-t}=-\frac{\partial\log B(t,t)}{\partial T}.
\end{equation} 
\end{definice}
...
Při odvození její hodnoty využíváme pravidla o neexistenci arbitráže na finančním trhu. 
...
f(t,t_1)=\lim_{T\to t_1}F(t,t_1,T)=-\frac{\partial\log B(t,t_1)}{\partial T}.
\end{equation} 
\end{definice}
...
V další části této kapitoly budeme tedy předpokládat stochastické chování úrokové míry.
...
Popsat, že CVA se týká oceňování derivátů!

Při oceňování derivátů obvykle není bráno do úvahy riziko selhání protistrany

Cena by měla být upravena o hodnotu rizika selhání protistrany

Účetní ocenění finančních derivátů dle IFRS  \cite{ifrs13}.
...
D(t)=\frac{B_0}{B_t}=e^{-r_ft},
\end{equation}
where $r_f$ is risk-free rate of return and $B_t$ is the value of risk free asset at time $t$. 
...
\hline
\textbf{Maturita (v letech)} &\textbf{úroková míra (\%)}\\ 
\hline
0,25 & 0,29 \\% 0,14  \\ 
0,5 & 0,37 \\% 0,16  \\
1 & 0,21 \\
...
\item [$\theta(t)$] Drift function defined as
\begin{equation}
\theta(t)=\frac{\partial  F(0,t)}{\partial t}+\alpha F(0,t)+\frac{\sigma^2}{2 \alpha}(1-\mathrm{e}^{-2\alpha t}), %F'_{t}(0,t)+...
\end{equation}
where
...
V další části se generuje počáteční výnosová křivka, která se dále využívá při modelování úrokové míry pomocí Hull-Whitova modelu (viz Kapitola \ref{H-W_model_kap}).
