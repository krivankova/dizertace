...
Uvažujme $n$ stochastických procesů $X_1(t),\dots,X_n(t)$, které popisují nějakým způsobem související náhodné jevy, například vývoj cen $m$ různých akcií na kapitálovém trhu. 
...
X^i(t_0)&=X^i_0.\notag
\end{align}
kde $\boldsymbol{W}(\boldsymbol{t})=(W_1(t),\dots,W_m(t))^\mathrm{T}$ je $m$-rozměrný Wienerův proces.
\end{definice}
...
\begin{equation}\label{Lipschitzova_podminka}
|\boldsymbol\alpha( t,\boldsymbol x)|+|\boldsymbol{B}( t,\boldsymbol x)|\leq C(1+|\boldsymbol x|),
\end{equation}
kde $\boldsymbol x\in\mathbb{R}^n$ a $t\in T$, pro nějakou konstantu $C$.
\end{veta}
