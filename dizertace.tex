...
Merton předpokládá, že cena $i$-tého rizikového aktiva $P_i(t)$ se řídí stochastickým procesem
$$\mathrm{d}P_i(t)=P_i(t)\,\mu_i\,\mathrm{d}t+P_i(t)\,\sigma_i\,\mathrm{d}W_i(t)$$
kde $W_i(t)$ je jednorozměrný Wienerův proces a $\mu_i$, $\sigma_i$ jsou konstanty. 
...
kde pro všechny uvažované časy $t$ platí $\sum_{i=1}^n\tilde{\mu}_i(t)=1$.
...
\tilde{{\mu}_j}(t)=\frac{\sum\limits_{i=1}^n\sigma_{ji}P_i(t)N_i}{\sum\limits_{j=1}^n\sum\limits_{i=1}^n\sigma_{ji}P_i(t)N_i},
\end{equation}
...
Dále nebudeme uvažovat počty akcií a tudíž tržní hodnotu akcií ztotožníme s tržní cenou jedné akcie.
...
Odhad tohoto parametru provedeme výpočtem historické volatility z dat. 


Obrázek \ref{model_cen} a Obrázek \ref{model_cen_jednotlive}
...
Na Obrázku \ref{euler_milstein} vidíme srovnání Eulerovy numerické metody a Milsteinovy numerické metody, které byly popsány v Kapitole \ref{numericke_metody_kap}.
